...
pub fn handshake_test() {
    ...
    let kp1 =
        new_keypair_from_connect_magic1(CONNECT_MAGIC1_Noise_IK_25519_ChaChaPoly_BLAKE2s).unwrap();
...
pub fn noise_string_from_connect_magic1(magic: u64) -> Option<&'static str> {
    match magic {
        CONNECT_MAGIC1_Noise_IK_25519_ChaChaPoly_BLAKE2s => {
            Some("Noise_IK_25519_ChaChaPoly_BLAKE2s")
        }
        CONNECT_MAGIC1_Noise_IK_25519_ChaChaPoly_BLAKE2b => {
            Some("Noise_IK_25519_ChaChaPoly_BLAKE2b")
        }
...
pub fn new_keypair_from_connect_magic1(magic1: u64) -> Option<IdentityKeyPair> {
    if magic1 == CONNECT_MAGIC1_PLAIN_TEXT {
        let mut ret =
            new_keypair_from_connect_magic1(CONNECT_MAGIC1_Noise_IK_25519_ChaChaPoly_BLAKE2s)?;
        ret.magic1 = CONNECT_MAGIC1_PLAIN_TEXT;
        return Some(ret);
    }
    if let Some(noise_string) = noise_string_from_connect_magic1(magic1) {
        let kp = snow::Builder::new(noise_string.parse().ok()?)
            .generate_keypair()
            .ok()?;
...
pub fn new_keypair_from_connect_magic1_with_seed(
    magic1: u64,
    seed: [u8; 32],
) -> Option<IdentityKeyPair> {
    if magic1 == CONNECT_MAGIC1_PLAIN_TEXT {
        let mut ret = new_keypair_from_connect_magic1_with_seed(
            CONNECT_MAGIC1_Noise_IK_25519_ChaChaPoly_BLAKE2s,
            seed,
        )?;
        ret.magic1 = CONNECT_MAGIC1_PLAIN_TEXT;
    ...
    if let Some(noise_string) = noise_string_from_connect_magic1(magic1) {
        ...
        let kp = snow::Builder::with_resolver(
            noise_string.parse().ok()?,
            Box::new(SnowRngResolver {
                rng: RustIsBadRngWrapper(rng),
            }),
        )
        .generate_keypair()
        .ok()?;
...
impl STPAddress {
    ...
    pub fn is_ipv6(&self) -> bool {
        self.ip.to_ipv4_mapped().is_none()
    }
    pub fn from(ip: Ipv6Addr, port: u16, kp: &IdentityKeyPair) -> Self {
        Self {
            ip,
            port,
            magic1: kp.magic1,
    ...
    pub fn parse(addr: &str) -> Option<STPAddress> {
        ...
        let b64_magic1 = parts.next()?;
        if b64_magic1.contains('=') {
            return None;
        }
        let b64_key = parts.next()?;
        if b64_key.contains('=') {
            return None;
        }
        let magic1_bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .decode(b64_magic1)
            .ok()?;
        if magic1_bytes.len() != 6 {
            return None;
        }
        let key = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .decode(b64_key)
            .ok()?;
        if key.len() < MIN_PUBLIC_KEY_SIZE || key.len() > MAX_PUBLIC_KEY_SIZE {
            return None;
        }
        let mut buf = [0u8; 8];
        buf[..6].copy_from_slice(&magic1_bytes);
        let magic1 = u64::from_le_bytes(buf);
        if magic1 & 1 == 0 {
            return None;
        }
        Some(STPAddress {
...
pub fn fmt_prefixed_byte_str_rev(
    f: &mut std::fmt::Formatter<'_>,
    pre: &str,
    bytes: &[u8],
) -> std::fmt::Result {
...
impl From<&STPAddress> for ConnectionKey {
    fn from(a: &STPAddress) -> Self {
        Self {
            ip: a.ip,
            port: a.port,
            key_15_bits: load_u16(&a.key[0..2]) << 1,
        }
    }
}
...
pub fn connect_to(
    ...
    if connections_map.contains_key(&key) {
        return Err(format!(
            "Error: Already connected or connecting to {:?}.",
            address
        )); // Ok(());
    }
    ...
    Err(format!(
        "Error: Can't connect to given peer: {:?}. No compatible keypair.",
        address
    )
    .to_string())
}
...
pub fn get_connected<'a>(
    m: &'a HashMap<ConnectionKey, ConnectionTrackingData>,
    k: &ConnectionKey,
) -> Option<&'a ConnectionTrackingData> {
...
pub fn get_connected_mut<'a>(
    m: &'a mut HashMap<ConnectionKey, ConnectionTrackingData>,
    k: &ConnectionKey,
) -> Option<&'a mut ConnectionTrackingData> {
...
pub fn connection_state_string(state: &ConnectionState) -> &'static str {
    match state {
        ConnectionState::SendingClientHelloPlaintext { .. } => {
            return "SendingClientHelloPlaintext";
        }
        ConnectionState::SendingClientHello { .. } => {
            return "SendingClientHello";
        }
        ConnectionState::SendingServerHelloPlaintext { .. } => {
            return "SendingServerHelloPlaintext";
        }
        ConnectionState::SendingServerHello { .. } => {
            return "SendingServerHello";
        }
...
pub fn do_the_test_program2(
    my_port: u16,
    my_listen_keypairs: Vec<&IdentityKeyPair>,
    beam_to: Option<(&IdentityKeyPair, &STPAddress)>,
) {
        ...
        if my_connect_keypair.magic1 == CONNECT_MAGIC1_PLAIN_TEXT {
            ...
            connections_map.insert(
                ...
                ConnectionTrackingData {
                    ...
                    connection_state: ConnectionState::SendingClientHelloPlaintext {
                        last_sent_time_ns: 0,
                        hello_packet_payload,
                    },
                },
            );
        } else {
            ...
            let mut handshake = snow::Builder::new(
                noise_string_from_connect_magic1(my_connect_keypair.magic1)
                    .unwrap()
                    .parse()
                    .unwrap(),
            )
            .prologue(&hello_packet_payload[0..6])
            .unwrap()
            .local_private_key(&my_connect_keypair.private[..])
            .unwrap()
            .remote_public_key(&endpoint.key[..])
            .unwrap()
            .build_initiator()
            .unwrap();

            // TODO list protocols
            let handshake_size = handshake
                .write_message(&[], &mut hello_packet_payload[6..])
                .unwrap();
            ...
            connections_map.insert(
                ...
                ConnectionTrackingData {
                    ...
                    other_port: endpoint.port,
                    other_transport_identity: endpoint.key.clone(),
                    connection_state: ConnectionState::SendingClientHello {
                        last_sent_time_ns: 0,
                        hello_packet_payload,
                        handshake,
                    },
...
#[repr(u8)]
pod! { pub enum PackletTag { #[default] Acknowledgements = 0, AnEntireDatagram = 1, OneJumboFragment = 2, ReliableStreamed = 3 } }
#[repr(C)]
pod! { pub struct PackletHeader(u16); }
#[repr(C)]
pod! { pub struct PackletAcknowledgements { pub hdr: PackletHeader } }
#[repr(C)]
pod! { pub struct PackletAnEntireDatagram { pub hdr: PackletHeader } }
#[repr(C)]
pod! { pub struct PackletOneJumboFragment { pub hdr: PackletHeader, pub id_idx: u32 } }
#[repr(C)]
pod! { pub struct PackletReliableStreamed { pub hdr: PackletHeader, pub id: u32, pub seq: u32 } }

impl PackletHeader {
    pub fn new(tag: PackletTag, len: u16) -> Self {
        debug_assert!(len < (1 << 14));
        Self((tag as u16) | (len << 2))
    }
    pub fn tag(self) -> PackletTag {
        unsafe { core::mem::transmute((self.0 & 0x3) as u8) }
    }
    pub fn len(self) -> u16 {
        self.0 >> 2
    }
...
impl PackletOneJumboFragment {
    pub fn new(hdr: PackletHeader, id: u8, idx: u32) -> Self {
        debug_assert!(idx < (1 << 24));
        Self {
            hdr,
            id_idx: (id as u32) | (idx << 8),
        }
    }
    pub fn id(self) -> u8 {
        self.id_idx as u8
...
impl SliceRead for PackletHeader {
    fn read_from(buf: &mut &[u8]) -> Option<Self> {
        Some(Self(u16::read_from(buf)?))
    }
}
impl SliceRead for PackletAcknowledgements {
    fn read_from(buf: &mut &[u8]) -> Option<Self> {
        Some(Self {
...
impl SliceRead for PackletAnEntireDatagram {
    fn read_from(buf: &mut &[u8]) -> Option<Self> {
        Some(Self {
            hdr: PackletHeader::read_from(buf)?,
        })
...
impl SliceWrite for PackletHeader {
    fn write_to(&self, buf: &mut [u8]) -> usize {
        self.0.write_to(buf)
    }
}
impl SliceWrite for PackletAcknowledgements {
    fn write_to(&self, buf: &mut [u8]) -> usize {
        self.hdr.write_to(buf)
    }
}
impl SliceWrite for PackletAnEntireDatagram {
    fn write_to(&self, buf: &mut [u8]) -> usize {
        self.hdr.write_to(buf)
    }
}
...
pub fn service_connections(
    ...
    if let Ok((
        ...
        service_class,
        timestamp_ns,
    )) = udp_recv_with_congestion_and_dscp(socket, &mut packet_memory_encrypted[..])
    {
        result = true;
                ...
                if magic1 == CONNECT_MAGIC1_PLAIN_TEXT {
                        ...
                        if my_kp.magic1 == CONNECT_MAGIC1_PLAIN_TEXT {
                            if buf_len >= 6 + 32 {
                                ...
                                let connection_key = ConnectionKey {
                                    ip: other_ip_addr,
                                    port: other_port,
                                    key_15_bits: load_u16(&client_key[0..2]) << 1,
                                };
                                if let Some(existing_connection) =
                                    connections_map.get_mut(&connection_key)
                                {
                                    if &existing_connection.my_transport_identity_keypair == my_kp
                                        && existing_connection.other_transport_identity
                                            == client_key
                                    {
                                        if let ConnectionState::SendingClientHelloPlaintext {
                                            last_sent_time_ns,
                                            hello_packet_payload,
                                        } = &mut existing_connection.connection_state
                                        {
                                            if *client_key < *my_kp.public {
                                                store_u48(
                                                    &mut packet_memory_send[0..6],
                                                    0xffff_ffff_0000
                                                        | (load_u16(&my_kp.public[0..2]) << 1)
                                                            as u64,
                                                );
                                                packet_memory_send[6..6 + 32]
                                                    .copy_from_slice(&my_kp.public[..]);
                                                // TODO single chosen Application Level protocols for e.g. zcash network upgrades.
                                                let hello_packet_payload =
                                                    Vec::from(&packet_memory_send[0..6 + 32]);

                                                existing_connection.connection_state =
                                                    ConnectionState::SendingServerHelloPlaintext {
                                                        last_sent_time_ns: 0,
                                                        hello_packet_payload,
                                                    };
                                                if OVERLY_VERBOSE {
                                                    println!(
                                                        "Transitioned connection {} to SendingServerHelloPlaintext.",
                                                        connection_key.key_15_bits
                                                    );
                                                }
                                            } else {
                                                if OVERLY_VERBOSE {
                                                    println!(
                                                        "Did NOT respond to connection {}: Lost the lexicographic compare.",
                                                        connection_key.key_15_bits
                                                    );
                                                }
                                            }
                                        } else {
                                            let state_str = connection_state_string(
                                                &existing_connection.connection_state,
                                            );
                                            if OVERLY_VERBOSE {
                                                println!(
                                                    "Did NOT respond to connection {}: We were not in SendingClientHelloPlaintext state. We are in {} state.",
                                                    connection_key.key_15_bits, state_str
                                                );
                                            }
                                        }
                                    } else {
                                        if OVERLY_VERBOSE {
                                            println!(
                                                "Did NOT respond to connection {}: Failed condition \"&existing_connection.my_transport_identity_keypair == my_kp && existing_connection.other_transport_identity == client_key\"",
                                                connection_key.key_15_bits
                                            );
                                        }
                                    }
                                    ...
                                    store_u48(
                                        &mut packet_memory_send[0..6],
                                        0xffff_ffff_0000
                                            | (load_u16(&my_kp.public[0..2]) << 1) as u64,
                                    );
                                    packet_memory_send[6..6 + 32]
                                        .copy_from_slice(&my_kp.public[..]);
                                    // TODO single chosen Application Level protocols for e.g. zcash network upgrades.
                                    ...
                                    let my_ip = if other_ip_addr.to_ipv4_mapped().is_some() {
                                        Ipv6Addr::UNSPECIFIED
                                    } else {
                                        Ipv6Addr::UNSPECIFIED
                                    };
                                    connections_map.insert(
                                        ...
                                        ConnectionTrackingData {
                                            ...
                                            two_byte_send_prefix: load_u16(&my_kp.public[0..2])
                                                << 1,
                                ...
                                if OVERLY_VERBOSE {
                                    println!(
                                        "Did NOT respond to connection: The packet did not contain 32 bytes of key. It was {} bytes.",
                                        buf_len
                                    );
                                }
                            }
                    ...
                    for key_i in 0..my_listen_keypairs.len() {
                        ...
                        if my_kp.magic1 == magic1 {
                            let mut new_handshake =
                                snow::Builder::new(noise_string.parse().unwrap())
                                    .prologue(&packet_memory_encrypted[0..6])
                                    .unwrap()
                                    .local_private_key(&my_kp.private)
                                    .unwrap()
                                    .build_responder()
                                    .unwrap();
                            if let Ok(list_of_protocols_len_bytes) = new_handshake.read_message(
                                &packet_memory_encrypted[6..buf_len],
                                &mut packet_memory_recv[..],
                            ) {
                                if let Some(client_key) = new_handshake.get_remote_static() {
                                    ...
                                    if let Some(existing_connection) =
                                        connections_map.get_mut(&connection_key)
                                    {
                                        if &existing_connection.my_transport_identity_keypair
                                            == my_kp
                                            && existing_connection.other_transport_identity
                                                == client_key
                                        {
                                            if let ConnectionState::SendingClientHello {
                                                last_sent_time_ns,
                                                hello_packet_payload,
                                                handshake,
                                            } = &mut existing_connection.connection_state
                                            {
                                                if *client_key < *my_kp.public {
                                                    ...
                                                    // TODO single chosen Application Level protocols for e.g. zcash network upgrades.
                                                    let handshake_size = new_handshake
                                                        .write_message(
                                                            &[],
                                                            &mut packet_memory_send[6..],
                                                        )
                                                        .unwrap();
                                                    let hello_packet_payload = Vec::from(
                                                        &packet_memory_send[0..6 + handshake_size],
                                                    );

                                                    debug_assert!(
                                                        new_handshake.is_handshake_finished()
                                                    );
                                        ...
                                        // TODO single chosen Application Level protocols for e.g. zcash network upgrades.
                                        let handshake_size = new_handshake
                                            .write_message(&[], &mut packet_memory_send[6..])
                                            .unwrap();
                                        let hello_packet_payload =
                                            Vec::from(&packet_memory_send[0..6 + handshake_size]);

                                        debug_assert!(new_handshake.is_handshake_finished());
                                        let cipher =
                                            ConnectionCipherTriplet::new_from_old_init_only(
                                                new_handshake
                                                    .into_stateless_transport_mode()
                                                    .expect("Cannot fail given assert above."),
                                            );
                ...
                loop {
                    ...
                    match &mut existing_connection.connection_state {
                        ...
                        ConnectionState::SendingClientHello {
                            ...
                            if magic1 >> 16 == 0xffff_ffff && buf_len >= 6 + 32 {
                                if let Ok(payload_len) = handshake.read_message(
                                    &packet_memory_encrypted[6..buf_len],
                                    &mut packet_memory_recv[..],
                                ) {
                                    ...
                                    if VERBOSE {
                                        println!(
                                            ...
                                            handshake.get_remote_static().unwrap()
                                        );
                                    }
                                    ...
                                    let ConnectionState::SendingClientHello { handshake, .. } =
                                        old_state
                                    else {
                                        panic!();
                                    };
                                    let ConnectionState::Connected { cipher, .. } =
                                        &mut existing_connection.connection_state
                                    else {
                                        panic!();
                                    };
                                    *cipher =
                                        Some(ConnectionCipherTriplet::new_from_old_init_only(
                                            handshake
                                                .into_stateless_transport_mode()
                                                .expect("Cannot fail given assert above."),
                                        ));
                    ...
                    } = &mut existing_connection.connection_state
                    {
                        ...
                        if let Some(cipher) = cipher {
                            if let Ok(payload_len) = cipher.current.read_message(
                                non_virtual_nonce,
                                &packet_memory_encrypted[6..buf_len],
                                &mut packet_memory_recv[..],
                            ) {
                                payload = &packet_memory_recv[0..payload_len];
                            } else if let Ok(payload_len) = cipher.old.read_message(
                                non_virtual_nonce,
                                &packet_memory_encrypted[6..buf_len],
                                &mut packet_memory_recv[..],
                            ) {
                                payload = &packet_memory_recv[0..payload_len];
                            } else if let Ok(payload_len) = cipher.new.read_message(
                                non_virtual_nonce,
                                &packet_memory_encrypted[6..buf_len],
                                &mut packet_memory_recv[..],
                            ) {
                        ...
                        *recv_time_ns = timestamp_ns;
                        ...
                        if OVERLY_VERBOSE {
                            println!(
                                "Got data from {:?}  data: {:?}",
                                existing_connection.other_transport_identity, payload
                            );
                        }
                        ...
                        if OVERLY_VERBOSE {
                            println!(
                                "\x1b[32mGot data\x1b[0m from {:?}. Now {} packets received!", // "  data: {:?}"
                                existing_connection.address(),
                                packets_received_this_call.len()
                            );
                        }
                    }
                    break;
    ...
    connections_map.retain(|connection_key, connection_tracking_data| {
        match &mut connection_tracking_data.connection_state {
            ConnectionState::SendingClientHelloPlaintext {
                last_sent_time_ns,
                hello_packet_payload,
            } => {
                if *last_sent_time_ns + 2_500_000_000 < current_time_now_ns {
                    udp_send_with_congestion_and_dscp(
                        socket,
                        connection_tracking_data.other_ip,
                        connection_tracking_data.other_port,
                        &hello_packet_payload,
                        Dscp::Af21,
                    );
                    if *last_sent_time_ns == 0 {
                        // cheeky optimization
            ...
            ConnectionState::Connected {
                ...
                if *last_sent_keep_alive_time_ns + 5_000_000_000 < current_time_now_ns {
                    ...
                    store_u16(
                        &mut packet_memory_encrypted[0..2],
                        connection_tracking_data.two_byte_send_prefix,
                    );
                    ...
                    *send_sequence_number += 1;
                    ...
                    if let Some(cipher) = cipher {
                        packet_len = cipher
                            .current
                            .write_message(virtual_nonce, &[], &mut packet_memory_encrypted[6..])
                            .unwrap();
                    } else {
                        packet_len = 0;
                    }
                    ...
                    udp_send_with_congestion_and_dscp(
                        ...
                        connection_tracking_data.other_ip,
                        connection_tracking_data.other_port,
                        &packet_memory_encrypted[0..6 + packet_len],
                        Dscp::Af21,
                    );
                ...
                if *recv_time_ns + 15_000_000_000 < current_time_now_ns {
                    if VERBOSE {
                        println!(
                            "Disconnected from: {:?}.",
                            connection_tracking_data.other_transport_identity
                        );
                    }
    ...
    for (connection_key, data) in packets_to_send {
        let Some(ref mut connection) = connections_map.get_mut(connection_key) else {
            continue;
        };
        ...
        let ConnectionState::Connected {
            ...
            last_sent_keep_alive_time_ns,
            recv_time_ns,
        } = &mut connection.connection_state
        else {
            continue;
        };
        ...
        store_u16(
            &mut packet_memory_encrypted[0..2],
            connection.two_byte_send_prefix,
        );
        store_u32(&mut packet_memory_encrypted[2..6], virtual_nonce as u32);
        *send_sequence_number += 1;
        ...
        if let Some(cipher) = cipher {
            packet_len = cipher
                .current
                .write_message(virtual_nonce, &data, &mut packet_memory_encrypted[6..])
                .unwrap();
        } else {
            packet_len = data.len();
...
pub fn do_the_test_program(port: u16, beam_to: Option<(Ipv6Addr, u16)>) {
    ...
    pub fn send_acks_helper(ack_state: &mut AckState, send_state: &mut SendState, ecn_down: bool) {
        ...
        if send_state.serial_number + 1
            >= send_state.drop_cursor + (PACKET_HISTORY_BUFFER_LEN as u64)
        {
        ...
        let mut o = 9;
        store_u64(
            &mut ack_state.ack_send_buf[o..o + 8],
            (ack_state.acks_in_waiting_min & 0x7fff_ffff_ffff_ffff) | ((ecn_down as u64) << 63),
        );
        o += 8;
        for i in 0..ack_state.acks_in_waiting_count {
            let val = ((ack_state.acks_in_waiting_buf[i].0 - ack_state.acks_in_waiting_min) as u32
                & 0x7f_ffff)
                | ((ack_state.acks_in_waiting_buf[i].1 as u32) << 23);
            store_u24(&mut ack_state.ack_send_buf[o..o + 3], val);
            o += 3;
        }
        let res = udp_send_with_congestion_and_dscp(
            send_state.socket,
            ack_state.saved_other_ip_addr,
            ack_state.saved_other_port,
            &ack_state.ack_send_buf[0..o],
            Dscp::Af21,
        );
        if let Ok(_timestamp_ns) = res {
            send_state.packet_buffer
                [send_state.serial_number as usize % PACKET_HISTORY_BUFFER_LEN] = u32::MAX;
    ...
    loop {
        ...
        if ack_state.acks_in_waiting_count > 0
            && monotonic_clock_ns() - ack_state.first_waiting_ack_time_ns
        ...
        let bottleneck_bandwidth_Bps =
            (current_max_delivered_bucket_bytes * 1_000_000_000) / data_delivery_bucket_time;
        ...
        if measured_allowed_bytes_on_the_wire > old_measured_allowed_bytes_on_the_wire * 124 / 100 {
        ...
        let allowed_bytes_on_the_wire = if state_machine_cursor < 2 {
            (measured_allowed_bytes_on_the_wire * 130 / 100)
                .max(measured_allowed_bytes_on_the_wire + tu_bytes * 10)
        } else if state_machine_cursor < 10 {
            measured_allowed_bytes_on_the_wire
        } else {
            (measured_allowed_bytes_on_the_wire * 125 / 100)
                .max(measured_allowed_bytes_on_the_wire + tu_bytes)
        };

        if time_of_last_status_print.elapsed() > std::time::Duration::from_millis(1000) {
            time_of_last_status_print = std::time::Instant::now();
            println!(
                "ecn up/down:{}/{} rtt: {} us MaxBucket: {} B bottleneck bandwidth: {}",
                ecn_up as u8,
                ecn_down as u8,
        ...
        while send_state.drop_cursor < send_state.serial_number {
            // The drop back edge.
            let stored_int = send_state.packet_buffer
                [send_state.drop_cursor as usize % PACKET_HISTORY_BUFFER_LEN];
            if stored_int != u32::MAX {
                let (packet_size_bytes, send_timestamp_ns, _ecn_marked, acked) =
                    decompress_packet_info(stored_int);
                ...
                if time_since_send_ns
                    < data_delivery_bucket_time * bytes_delivered_buckets.len() as u64
                {
                    break;
                }
                if acked == false {
            ...
            >= send_state.drop_cursor + (PACKET_HISTORY_BUFFER_LEN as u64)
        {
            ...
            let to_send_len_compressed =
                decompress_packet_size_to_8_bits(compress_packet_size_to_8_bits(tu_bytes as u16))
                    as u64;
            if to_send_len_compressed + bytes_on_the_wire <= allowed_bytes_on_the_wire {
                if let Some((beam_to_ip, beam_to_port)) = beam_to {
                    ...
                    if let Ok(timestamp_ns) = res {
                        send_state.packet_buffer
                            [send_state.serial_number as usize % PACKET_HISTORY_BUFFER_LEN] =
        ...
        if packet_plaintext[0] == 2 {
            ...
            while o < packet_plaintext.len() {
                ...
                let (packet_size_bytes, send_timestamp_ns, is_mtu_poll, acked) =
                    decompress_packet_info(
                        send_state.packet_buffer[ack_number as usize % PACKET_HISTORY_BUFFER_LEN],
                    );
                if acked {
            ...
            if total_bytes_acked_this_ack > 0 {
                ...
                if current_time_ns
                    > bytes_delivered_bucket_cursor_last_time + data_delivery_bucket_time
                {
            ...
            ack_state.acks_in_waiting_buf[ack_state.acks_in_waiting_count] =
                (packet_serial, ecn_marked);
            ack_state.acks_in_waiting_count += 1;
            if ack_state.acks_in_waiting_count == ASSUMED_ACK_CAPACITY
                || monotonic_clock_ns() - ack_state.first_waiting_ack_time_ns
