...
use zcash_protocol::consensus::{BlockHeight as LRZBlockHeight, BranchId, Network, NetworkType};
...
use zcash_protocol::{
    consensus::{Parameters, MAIN_NETWORK, TEST_NETWORK},
...
impl WalletState {
    ...
    pub fn send_to_address(&mut self, address: String, amount: u64) {
        let address = match self.network {
            NetworkType::Main => {
                let Ok(address) = UnifiedAddress::decode(&MAIN_NETWORK /* @todo */, &address)
                else {
            ...
            NetworkType::Test => {
                let Ok(address) = UnifiedAddress::decode(&TEST_NETWORK /* @todo */, &address)
                else {
                    println!("Invalid address for send: {}", address);
                    return;
                };
            ...
            NetworkType::Regtest => {
                let Ok(address) =
                    UnifiedAddress::decode(&REGTEST_NETWORK /* @todo */, &address)
                else {
                    println!("Invalid address for send: {}", address);
                    return;
                };
...
impl ManualWallet {
    ...
    async fn send_built_tx<P: Parameters>(
        ...
        if let Err(err) = tx.write(&mut raw_tx.data) {
            ...
            if DUMP_TX_SEND {
                println!("******* res for {:?} ({} B): {:?}", tx.txid(), tx_size, res);
            }
...
pub async fn wallet_main(wallet_state: Arc<Mutex<WalletState>>) {
    ...
    let wallet_guard = wallet_state.lock().unwrap();
    let wallet_network = wallet_guard.network.clone();
    drop(wallet_guard);

    *FAUCET_REQUEST.lock().unwrap() = Some(FaucetRequestClosure(Arc::new(
        move |ua_str: String| -> Result<u64, String> {
            let ua = match wallet_network {
                NetworkType::Main => {
                    println!("wallet network is main");
                    match zcash_keys::address::Address::decode(&MAIN_NETWORK, &ua_str) {
                ...
                NetworkType::Test => {
                    println!("wallet network is TEST");

                    match zcash_keys::address::Address::decode(&TEST_NETWORK, &ua_str) {
                        Some(zcash_keys::address::Address::Unified(ua)) => ua,
                        Some(_) => return Err(format!("must be an orchard-containing UA")),
                        None => return Err(format!("couldn't decode address")),
                    }
                ...
                NetworkType::Regtest => {
                    println!("wallet network is REGTEST");

                    match zcash_keys::address::Address::decode(&REGTEST_NETWORK, &ua_str) {
                        Some(zcash_keys::address::Address::Unified(ua)) => ua,
                        Some(_) => return Err(format!("must be an orchard-containing UA")),
                        None => return Err(format!("couldn't decode address")),
                    }
